...
def upload_model(unique_id, data_dir):
    ...
    model_result_interface["domain_idx_1"] = model_result_interface["idxs"].apply(lambda x: x[0])
    model_result_interface["domain_idx_2"] = model_result_interface["idxs"].apply(lambda x: x[1])

    model_result_interface["domain_id_1"] = model_result_interface[
        ["protein_id_1", "domain_idx_1"]
    ].apply(lambda x: domain_id_lookup[tuple(x)], axis=1)
    model_result_interface["domain_id_2"] = model_result_interface[
...
def upload_mutation(unique_id, mutation, data_dir, elaspic_version=""):
    ...
    domain_id_lookup = get_domain_id_lookup(connection, unique_id)
    mutation_result_core["domain_id"] = mutation_result_core[["protein_id", "domain_idx"]].apply(
        lambda x: domain_id_lookup[tuple(x)], axis=1
    )
    ...
    mutation_result_interface["chain_idx"] = mutation_result_interface[["idx", "idxs"]].apply(
        get_chain_idx, axis=1
    )
    ...
    # protein idx
    mutation_result_interface["domain_idx_1"] = mutation_result_interface["idxs"].apply(
        lambda x: x[0]
    )
    mutation_result_interface["domain_idx_2"] = mutation_result_interface["idxs"].apply(
        lambda x: x[1]
    )
    # domain id
    mutation_result_interface["domain_id_1"] = mutation_result_interface[
        ["protein_id_1", "domain_idx_1"]
    ].apply(lambda x: domain_id_lookup[tuple(x)], axis=1)
    mutation_result_interface["domain_id_2"] = mutation_result_interface[
        ["protein_id_2", "domain_idx_2"]
    ].apply(lambda x: domain_id_lookup[tuple(x)], axis=1)
    # interface id
    mutation_result_interface["interface_id"] = mutation_result_interface[
